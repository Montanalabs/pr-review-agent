...
Untrusted input is a pull request; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a pull request
- **Closed type:** `type Decision = Comment(PrNote) | RequestChanges | ApprovePr` where `PrNote` is closed
- **Sink / capability:** `grant commentPr` → `privileged { commentPr(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/pr-review-agent/demo.sh
```
...
- `pr-review-agent_safe.eide` — the correct design.
- `pr-review-agent_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
